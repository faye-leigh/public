...
/**
 * CSE 2221 Project #9. Utility class to support string reassembly from
 * fragments.
 *
 * @author Faye Leigh
 *
...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
         ...
         */
        String combination = str1 + str2.substring(overlap);
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        for (String s1 : strSet) {
            for (String s2 : strSet) {
                if (!s1.equals(s2)) {
                    assert !s1.contains(
                            s2) : "Violation of: CONTAINS_NO_SUBSTRING_PAIRS(strSet)";
                }
            }
        }
        ...
        /*
         * Check if str is a substring of any element of s. If so, do not add
         * str to strSet
         */
        for (String s1 : strSet) {
            if (s1.contains(str)) {
        ...
        /*
         * Remove any substrings of str within strSet, then add str to strSet
         */
        if (add) {
            Set<String> tmp = strSet.newInstance();
            tmp.transferFrom(strSet);
            while (tmp.size() > 0) {
                String s = tmp.removeAny();
                if (!str.contains(s)) {
                    strSet.add(s);
                }
            }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        // Read each line of input and add to set.
        while (!input.atEOS()) {
            addToSetAvoidingSubstrings(lines, input.nextLine());
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int i = 0; i < text.length(); i++) {
            if (Character.compare(text.charAt(i), '~') != 0) {
    ...
    public static void main(String[] args) {
        ...
        SimpleReader inFile = new SimpleReader1L("data/" + inputFileName);
        /*
