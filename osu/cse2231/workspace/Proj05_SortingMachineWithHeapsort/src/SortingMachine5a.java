...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    * </pre>
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (last > top * 2) {
            int child = top * 2 + 1;
            /*
             * If both left and right subtrees exist, use the smaller of the
             * two.
             */
            if (child < top
                    && order.compare(array[child + 1], array[child]) < 0) {
                child++;
            }
            /*
             * If root node is larger than the smaller subtree node, swap the
             * values and recursively call siftDown on the child subtree.
             */
            if (order.compare(array[top], array[child]) > 0) {
                exchangeEntries(array, top, child);
                siftDown(array, child, last, order);
            }
        }
    }
    ...
    * </pre>
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
         ...
         */
        /*
         * If root node has no subtrees, method ends.
         */
        if (array.length - 1 > top * 2) {
            /*
             * Find index for left subtree and recursively call heapify on it.
             */
            int child = top * 2 + 1;
            heapify(array, child, order);
            /*
             * If right subtree also exists, recursively call heapify on it.
             */
            if (child < array.length - 1) {
                heapify(array, child + 1, order);
            }
            /*
             * Move root node to correct position to heapify full tree.
             */
    ...
    * </pre>
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        /*
         * Add all entries of q to array and return after heapifying.
         */
    ...
    * </pre>
    ...
    public final void changeToExtractionMode() {
        ...
        /*
         * Move all entries of insertion queue into heap array.
         */
    ...
    public final T removeFirst() {
        ...
        /*
         * Re-heapify heap with second entry as root node. Then move first entry
         * to rear of heap array without changing order of other entries,
         * effectively removing the first entry.
         */
        heapify(this.heap, 1, this.machineOrder);
        for (int i = 0; i < this.heapSize - 1; i++) {
            exchangeEntries(this.heap, i, i + 1);
        }
        this.heapSize--;

        assert this.conventionHolds();

        return this.heap[this.heapSize]; // New last entry = old first entry.
    }
    ...
    public final int size() {
        /*
         * Return insertion queue length if in insertion mode, otherwise return
         * heapSize.
         */
