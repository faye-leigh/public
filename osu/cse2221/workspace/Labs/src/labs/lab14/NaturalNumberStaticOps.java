...
/**
 * CSE 2221 Lab #14. Program with implementation of some {@code NaturalNumber}
 * secondary operations implemented as static methods: increment, decrement, and
...
public final class NaturalNumberStaticOps {
    ...
    /**
     * Decrements the given {@code NaturalNumber}. 10
     *
     * @param n
     ...
     * @ensures out.content = #out.content * [display of n with commas] 123456
     */
    private static void printWithCommas(NaturalNumber n, SimpleWriter out) {
        ...
        if (n.compareTo(ONE_THOUSAND) > -1) {
            NaturalNumber digits = n.divide(ONE_THOUSAND);
            printWithCommas(n, out);
            out.print("," + digits);
            n.multiply(ONE_THOUSAND);
            n.add(digits);
        } else {
            out.print(n);
        }
    ...
    private static String toStringWithCommas(NaturalNumber n) {
        ...
        if (n.compareTo(ONE_THOUSAND) > -1) {
            NaturalNumber digits = n.divide(ONE_THOUSAND);
            output = toStringWithCommas(n) + "," + digits;
            n.multiply(ONE_THOUSAND);
            n.add(digits);
        } else {
            output = n.toString();
